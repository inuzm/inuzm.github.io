...
void updateparticle() {
  ...
  if ( (ypart - 0.75 * float(dir1) <= 570) & (ypart  - 0.75 * float(dir1) >= 30) ) {
    movery = false;
    ypart -= 0.75 * float(dir1);
  } else {
    movery = true;
  }
}
...
void updateytraj() {
  ...
  if ( xpart < 570 ){
    ...
    if ( (ypart - 0.75 * float(dir1) <= 570) & (ypart  - 0.75 * float(dir1) >= 30) ) {
      ay[0] += 0.75 * float(getsign(ay[0], ay[1]));
      ay[7] = ypart;
    } else {
